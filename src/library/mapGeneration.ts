...
const phaserGridGenerator = (width: number, height: number): PhaserMap => {
  let overAllArea = generateNewMap(width, height);
  let randomCutCounts = getRandomIntInclusive(5, 9);
  let dividedArea = getLeaf(mapRandomizerBreadth(overAllArea, randomCutCounts));
  let rooms = placeRoom(dividedArea);
  let tunnels = connectRoom(rooms);
  return { rooms: rooms, tunnels: tunnels };
};

const randomSpawnPoint = (mapForPhaser: PhaserMap): number[] => {
  let randomPoint = getRandomIntInclusive(0, mapForPhaser.rooms.length) - 1;
  let chosenRoom = mapForPhaser.rooms[randomPoint];
  let randomXCordinate =
    getRandomIntInclusive(chosenRoom.xAxis[0], chosenRoom.xAxis[1]) - 1;
  let randomYCordinate =
    getRandomIntInclusive(chosenRoom.yAxis[0], chosenRoom.yAxis[1]) - 1;
  return [randomXCordinate, randomYCordinate];
};
